...
//   { icon: 'code', value: '150K+', label: 'Lines of Code' },
...
export default function ProjectsHero() {
  return (
    <section className="relative px-6 md:px-24 pt-10 md:pt-16 pb-14 overflow-hidden">

      {/* decorative dot map — purely atmospheric, no real geodata */}
      <div className="hidden md:block absolute right-0 top-0 w-1/2 h-full opacity-40 pointer-events-none">
        <DotMap />
      </div>
      ...
      <motion.div
        ...
        className="relative flex flex-wrap gap-5 mt-10"
      >
        {stats.map((s) => (
          <div
            key={s.label}
...
function DotMap() {
  return (
    <svg viewBox="0 0 100 100" className="w-full h-full">
      {DOTS.map((d, i) => (
        <circle key={i} cx={d.x} cy={d.y} r={d.r} fill="#fbbf24" opacity={d.r > 1 ? 0.8 : 0.25} />
      ))}
    </svg>
  );
}